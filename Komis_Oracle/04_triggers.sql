...
-- 0.1) KOLUMNA AGREGATU (spójność pośrednia) - trigger utrzymujący agregat
ALTER TABLE plac ADD (liczba_aut NUMBER DEFAULT 0 NOT NULL);


-- 1) TRIGGER: Historia zmian ceny samochodu- trigger audytowo-logujący zmianę danych (PostgreSQL #1: log_cena_update)
...
-- 2) TRIGGER WALIDACYJNY transakcji -trigger walidacyjny (BEFORE INSERT/UPDATE) +walidacja daty (reguła biznesowa), (PostgreSQL #2: check_car_ready_for_sale)
...
-- 3) TRIGGER: Ustawienie auta jako gotowe po dostawie - spójność pośrednia (licznik aut na placu) (PostgreSQL #3: mark_car_ready_after_delivery)
...
-- 6) TRIGGER: Walidacja dat polisy - dodatkowa walidacja reguły biznesowej, (PostgreSQL #5: set_default_end_date)
